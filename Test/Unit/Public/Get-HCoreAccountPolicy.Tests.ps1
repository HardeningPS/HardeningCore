...
Describe -Name $describe -Tag 'Function', 'Public', $function -Fixture {
    ...
    $splat = @{
        ExpectedOutputTypeNames = 'System.Object[]'
        Help                    = $help
    }
    ...
    $splat = @{
        ExpectedParameterNames = @( 'Type', 'ConfigDirectory' )
        Help                   = $help
    }
    ...
    Context -Name "Return Type" -Fixture {
        $testCases = @(
            @{
                FoundReturnType    = ( Get-HCoreAccountPolicy -ErrorAction 'Stop' ).GetType().FullName
                ExpectedReturnType = 'System.Object[]'
            }
        )
