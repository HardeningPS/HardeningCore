...
Describe -Name $describe -Tag 'Function', 'Private', $function -Fixture {
    ...
    $splat = @{
        ExpectedOutputTypeNames = 'System.Management.Automation.PSCustomObject'
        Help                    = $help
    }
    ...
    $splat = @{
        ExpectedParameterNames = @( 'Name', 'Detail' )
        Help                   = $help
    }
    ...
    Context -Name "Return Type" -Fixture {
        ...
        $testCases = @(
            @{
                FoundReturnType    = ( Get-HCoreCache -Name 'MyTestCache' -ErrorAction 'Stop' ).GetType().FullName
